...
function baseCreateRenderer(
  ...
  const patch: PatchFn = (
    ...
    */
    ...
    // set ref
    /* 
      每次patch都需要重新设置ref，这也就是为什么在编译阶段不对含有ref的节点进行提升的原因
      因为ref是当做动态属性来看待
    */
  ...
  const patchElement = (
    ...
    // 如果n2.patchFlag和n1的相同，取改flag，否则取FULL_PROPS
    ...
    // 运行时指令，执行元素上绑定的指令的钩子beforeUpdate
    if (dirs) {
    ...
    if (patchFlag > 0) {
      ...
      if (patchFlag & PatchFlags.FULL_PROPS) {
        ...
        */
        ...
        /* 
          除style、class外的动态属性，对dynamicProps中收集的动态属性做diff
          PROPS 的比较会将新节点的动态属性提取出来，并遍历这个这个属性中所有的 key
          当新旧属性不一致，或者该 key 需要强制更新时，则调用 hostPatchProp 对属性进行更新
        */
    ...
    if ((vnodeHook = newProps.onVnodeUpdated) || dirs) {
      /* 
        ...
        由scheduler调度器来控制执行时机的，effect先入队，在nextTick执行
      */
      queuePostRenderEffect(() => {
  ...
  const patchBlockChildren: PatchBlockChildrenFn = (
    ...
    for (let i = 0; i < newChildren.length; i++) {
      ...
      /* 
        决定子节点patch时的实际父节点容器
        三种情况需要访问实际的父节点dom：fragment、不同vnode、组件
      */
      const container =
        ...
        /* 
          ...
          在进行fragment diff时会涉及到节点的增删、移位、这种情况是需要通过真实父节点容器来操作的
          因此需要提供fragment节点原来的父级容器
        */
        (oldVNode.type === Fragment ||
          // - In the case of different nodes, there is going to be a replacement
          // which also requires the correct parent container
          // 非相同类型的vnode在patch时要替换节点，因此需要提供真实的父节点
          ...
          : // In other cases, the parent container is not actually used so we
            ...
            /* 
              翻：其他情况下，patch操作并不会涉及到使用父级dom容器，处于性能考虑，
  ...
  const processFragment = (
    ...
    /* 
      ...
      fragment将vnode的el作为起始定位节点，vnode的anchor作为结束定位节点
      fragment vnode和普通单节点有一点区别：其el并非fragment对应的真实dom，
      因为fragment自身没有一个真正的实体dom，因此是将fragment chidren在dom中的前一个元素作为el，结尾后一个元素作为anchor
      起到一个定位效果，方便用nextSibling
    */
    ...
    if (n1 == null) {
      ...
      if (
        ...
        /* 
          稳定的fragment，如template多根节点，或者<div v-for="i in 10">{{ i }}</div>
        ...
        */
        ...
        /*
          非稳定的fragment，比如<div v-for="i in dynanicArr">{{ i }}</div>
          虽然v-for的渲染单元结构固定，但是dynanicArr数组是会变化的，因此会导致
          fragment的子节点顺序和内容的不确定性，所以直接将fragment子节点
          作为dynamicChildren比较明显是不正确的：
          比如dynanicArr从[1, 2]变成[3, 4, 2, 1]，假如你给每个子节点的key是非数组index
          而是和数组元素内容强相关的元素值，key组：1 - 2 -> 3 - 4 - 2 - 1
          那么直接用patchDynamicChildren的话，diff会忽略节点顺序
          这么比较会导致更新错乱
          那没办法，只能走children的全量diff了，因此不稳定的fragment不会挂载dynamicChildren
          虽然fragment本身是block，需要注意的是，不稳定fragment的每个子节点都是block，保证子节点可以
          继续走优化diff
        */
        patchChildren(
          n1,
  ...
  const patchKeyedChildren = (
    ...
    /* 
      case1 头部向尾部遍历
      从节点组头部向尾部遍历，遇到尾指针则停止
      遍历过程中，遇到相似节点(tag、key均相等)直接patch比对
      否则退出遍历，此时i记录了diff最新的头部推进指针
    */
    while (i <= e1 && i <= e2) {
      const n1 = c1[i]
    ...
    /* 
      ...
      遍历过程中，遇到相似节点(tag、key均相等)直接patch比对，否则退出遍历
      此时e1，e2记录了diff最新的尾部推进指针
    */
    while (i <= e1 && i <= e2) {
      const n1 = c1[e1]
    ...
    /* 
      ...
      经过上面(case1、case2)的首尾夹击操作后，如果start -> end 和 end -> start
    ...
    */
    ...
    /* 
      ...
      经过(case1、case2)的首尾夹击操作后，如果start -> end 和 end -> start
      两个方向至少有一个遍历没有中途断掉，那么首尾指针便会相撞
    ...
    */
    ...
    /* 
      case5 存在未知序列
      该case下，start -> end 和 end -> start 两个方向在遍历中途均断掉
      (由于tag或key不相等)，导致新旧节点组中间均出现了未进行patch操作的节点序列
    */
    else {
      const s1 = i // prev starting index // 旧未知序列头指针
      const s2 = i // next starting index // 新未知序列头指针

      // 5.1 build key:index map for newChildren
      // 遍历新未知序列，记录新序列中节点key - index 键值对
      ...
      */
      ...
      /* 
        ...
        所以用数组来当做map来记录，用数组记录也是为了用于创建最长稳定子序列
        0为初始值，即表示新节点无对应的旧节点
      */
